...
namespace litehtml
{
	...
	public:
		...
		// Release all runtime state (hover/active elements, render tree) so the
		// document can be fully freed without waiting for Lua GC finalisation.
		void							clear_state() {
			m_over_element.reset();
			m_active_element.reset();
