...
def main():
    ...
    with open(OUT_DOCS, "w", encoding="utf-8") as f:
        ...
        f.write("| `SDL.Window.GetWindowWidth()` | — | `integer` | Current window width in pixels |\n")
        f.write("| `SDL.Window.GetWindowHeight()` | — | `integer` | Current window height in pixels |\n")
        f.write("| `SDL.Window.GetWindowX()` | — | `integer` | Window X position on screen |\n")
        f.write("| `SDL.Window.GetWindowY()` | — | `integer` | Window Y position on screen |\n")
        f.write("| `SDL.Window.SetWindowSize(w, h)` | `integer, integer` | — | Resize the window |\n")
        f.write("| `SDL.Window.SetWindowPosition(x, y)` | `integer, integer` | — | Move the window |\n")
        f.write("| `SDL.Window.SetWindowTitle(title)` | `string` | — | Change the window title bar text |\n")
        f.write("| `SDL.Window.IsMinimized()` | — | `boolean` | Whether the window is minimised |\n")
        f.write("| `SDL.Window.IsFocused()` | — | `boolean` | Whether the window has input focus |\n")
        f.write("| `SDL.Window.SetFullscreen(enabled)` | `boolean` | — | Toggle borderless fullscreen |\n")
        f.write("| `SDL.Window.GetMonitor()` | — | `index, name, x, y, width, height, refreshRate` | Info about the monitor the window is on |\n")
        f.write("\n")

        f.write("### SDL.Input\n\n")
        f.write("Polled each frame — not event-based. Safe to call at any point inside `renderFn`.\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `SDL.Input.GetKeyState(scancode)` | `integer` | `boolean` | Whether a key is currently held. Uses physical SDL_Scancode values (e.g. W=26, Space=44, Up=82, Left=80, Right=79, Down=81) |\n")
        f.write("| `SDL.Input.GetModState()` | — | `table` | `{ shift, ctrl, alt, gui, capslock, numlock }` — boolean fields |\n")
        f.write("| `SDL.Input.GetMouseState()` | — | `x, y, buttons` | Cursor position in window pixels and button bitmask. bit0=left, bit1=middle, bit2=right |\n")
        f.write("| `SDL.Input.GetRelativeMouseState()` | — | `dx, dy, buttons` | Pixel delta since last call and button bitmask. Resets to 0,0 each call |\n")
        f.write("| `SDL.Input.SetRelativeMouseMode(enabled)` | `boolean` | — | Capture and hide the cursor. Use `GetRelativeMouseState` for deltas |\n")
        f.write("| `SDL.Input.WarpMouse(x, y)` | `integer, integer` | — | Move the cursor to a window-relative position |\n")
        f.write("| `SDL.Input.GetNumJoysticks()` | — | `integer` | Number of connected joystick/gamepad devices |\n")
        f.write("| `SDL.Input.GetGamepadAxis(index, axis)` | `integer, integer` | `number` | Axis value normalised to -1.0..1.0. index is 0-based. Axis values match SDL_GameControllerAxis (0=LeftX, 1=LeftY, 2=RightX, 3=RightY, 4=TriggerLeft, 5=TriggerRight) |\n")
        f.write("| `SDL.Input.GetGamepadButton(index, button)` | `integer, integer` | `boolean` | Whether a button is held. index is 0-based. Button values match SDL_GameControllerButton (0=A, 1=B, 2=X, 3=Y) |\n")
        f.write("\n")

        f.write("### SDL.Time\n\n")
        f.write("Frame and high-resolution timing. Safe to call at any point inside `renderFn`.\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `SDL.Time.GetTicks()` | — | `integer` | Milliseconds since SDL was initialised |\n")
        f.write("| `SDL.Time.GetPerformanceCounter()` | — | `integer` | High-resolution timer counter value |\n")
        f.write("| `SDL.Time.GetPerformanceFrequency()` | — | `integer` | Ticks per second for the performance counter. Use with `GetPerformanceCounter` for sub-millisecond timing |\n")
        f.write("| `SDL.Time.GetFrameTime()` | — | `dt, fps` | `dt` = seconds since last frame (number). `fps` = exponential moving average frame rate. Both values are computed once at the start of each frame by the render loop — safe to call multiple times per frame |\n")
        f.write("\n")

        f.write("### SDL.Audio\n\n")
        f.write("Audio playback via SDL_mixer. Requires the engine to be compiled with `KITSUNE_AUDIO`. ")
        f.write("All functions require an active session (called after `Imgui.Start`). ")
        f.write("Audio ids (`luaId`) are stable integers assigned by the resource cache.\n\n")
        f.write("Sound effects (`SDL.Audio.Load`) are decoded fully into memory as `Mix_Chunk` and played on mixer channels. ")
        f.write("Music (`SDL.Audio.LoadMusic`) is streamed from memory as `Mix_Music` — only one music track plays at a time.\n\n")

        f.write("#### Loading & Lifecycle\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `SDL.Audio.Load(stream [, source])` | stream, `string?` | `integer` | Decode a sound effect from a stream (WAV, OGG, MP3, FLAC). If source matches a cached slot, replaces it keeping the same id |\n")
        f.write("| `SDL.Audio.LoadRaw(data [, source])` | `string`, `string?` | `integer` | Same as Load but accepts raw byte string instead of a stream |\n")
        f.write("| `SDL.Audio.LoadMusic(stream [, source])` | stream, `string?` | `integer` | Load a streaming music track from a stream |\n")
        f.write("| `SDL.Audio.Unload(luaId)` | `integer` | — | Free the `Mix_Chunk` but keep the cache slot as a sentinel |\n")
        f.write("| `SDL.Audio.Destroy(luaId)` | `integer` | — | Fully remove a sound-effect resource from the cache |\n")
        f.write("| `SDL.Audio.DestroyMusic(luaId)` | `integer` | — | Fully remove a music resource from the cache |\n")
        f.write("| `SDL.Audio.DestroyAll()` | — | — | Halt all playback and free every audio resource |\n")
        f.write("\n")

        f.write("#### Playback\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `SDL.Audio.Play(luaId [, loops])` | `integer, integer?` | `channel\\|nil` | Play a sound effect. `loops`: 0=once (default), -1=infinite, n=repeat n extra times. Returns the mixer channel or nil on failure |\n")
        f.write("| `SDL.Audio.PlayMusic(luaId [, loops])` | `integer, integer?` | `boolean` | Play a music track. `loops`: -1=infinite (default), 1=once |\n")
        f.write("| `SDL.Audio.Stop([luaId])` | `integer?` | — | Stop the channel the sfx is playing on. No argument stops all channels |\n")
        f.write("| `SDL.Audio.StopMusic()` | — | — | Stop music playback immediately |\n")
        f.write("| `SDL.Audio.FadeIn(luaId, ms [, loops])` | `integer, integer, integer?` | `channel\\|nil` | Fade in a sound effect over `ms` milliseconds |\n")
        f.write("| `SDL.Audio.FadeOut(luaId, ms)` | `integer, integer` | — | Fade out the channel the sfx is playing on over `ms` milliseconds |\n")
        f.write("| `SDL.Audio.PauseMusic()` | — | — | Pause the current music track |\n")
        f.write("| `SDL.Audio.ResumeMusic()` | — | — | Resume a paused music track |\n")
        f.write("\n")

        f.write("#### Volume\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `SDL.Audio.SetVolume(luaId, volume)` | `integer, integer` | — | Set per-chunk volume. Range 0–128 (`MIX_MAX_VOLUME`) |\n")
        f.write("| `SDL.Audio.SetMusicVolume(volume)` | `integer` | — | Set global music volume. Range 0–128 |\n")
        f.write("\n")

        f.write("#### Queries\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `SDL.Audio.IsPlaying(luaId)` | `integer` | `boolean` | Whether the channel this sfx last played on is still active |\n")
        f.write("| `SDL.Audio.IsMusicPlaying()` | — | `boolean` | Whether a music track is currently playing |\n")
        f.write("| `SDL.Audio.IsMusicPaused()` | — | `boolean` | Whether the current music track is paused |\n")
        f.write("| `SDL.Audio.GetId(source)` | `string` | `integer\\|nil` | Cached luaId for a source string. Checks sfx first, then music |\n")
        f.write("| `SDL.Audio.GetData(luaId)` | `integer` | `table\\|nil` | `{ source, isLoaded, type }` where `type` is `\"sfx\"` or `\"music\"` |\n")
        f.write("\n")

        f.write("## Resource API\n\n")
        f.write("Session-wide resource loader. Handles all resource types through a single callback so one function routes load requests for textures, audio, and any future resource types.\n\n")
        f.write("```lua\n")
        f.write("Resource.SetLoader(\n")
        f.write("    function(type, source)\n")
        f.write("        if type == 1 then -- RESOURCE_TEXTURE\n")
        f.write("            return Stream.Open(source, 'rb')\n")
        f.write("        elseif type == 2 then -- RESOURCE_AUDIO_SFX\n")
        f.write("            return Stream.Open(source, 'rb')\n")
        f.write("        end\n")
        f.write("        return nil\n")
        f.write("    end,\n")
        f.write("    function(type, luaId, source)  -- optional post-load callback\n")
        f.write("        if type == 1 then\n")
        f.write("            local data = OpenGL.GetData(luaId)\n")
        f.write("            if data and data.width > 1024 then\n")
        f.write("                OpenGL.UnloadTexture(luaId)\n")
        f.write("            end\n")
        f.write("        end\n")
        f.write("    end\n")
        f.write(")\n")
        f.write("```\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `Resource.SetLoader(loader [, postLoader])` | `function, function?` | — | Set the session-wide loader. `loader(type, source)` must return a stream or nil. `postLoader(type, luaId, source)` is called after a successful load |\n")
        f.write("\n")
        f.write("**Resource type constants passed to loader/postLoader:**\n\n")
        f.write("| Constant | Value | Consumer |\n|---|---|---|\n")
        f.write("| `RESOURCE_TEXTURE` | 1 | `OpenGL.LoadTexture`, `OpenGL.ResolveTexture` |\n")
        f.write("| `RESOURCE_AUDIO_SFX` | 2 | `SDL.Audio.Load` |\n")
        f.write("| `RESOURCE_AUDIO_MUSIC` | 3 | `SDL.Audio.LoadMusic` |\n")
        f.write("| `RESOURCE_FONT` | 4 | Font loading |\n")
        f.write("| `RESOURCE_GENERIC` | 5 | `Resource.Load`, `Resource.Resolve` — raw byte blobs |\n")
        f.write("| `RESOURCE_MARKDOWN` | 6 | `Markdown.Parse` |\n")
        f.write("| `RESOURCE_HTML` | 7 | `Html.Parse` |\n")
        f.write("\n")
        ...
        f.write("| `OpenGL.ResolveTexture(source)` | `string` | `integer\\|nil` | Cache-hit (live or sentinel) returns id immediately. On miss calls `Resource.SetLoader`. Returns `nil` only if source is completely unknown |\n")
        ...
        f.write("## Font API\n\n")
        f.write("Custom TTF fonts loaded into the ImGui atlas at runtime. ")
        f.write("Fonts are `RESOURCE_FONT` entries in the ResourceCache identified by an integer `luaId`. ")
        f.write("The source key format is `\"FaceName:size:style\"` (e.g. `\"Roboto:18.0:0\"`). ")
        f.write("Because ImGui bakes fonts into a GPU texture atlas, newly loaded fonts take effect on the frame *after* they are registered — ")
        f.write("the render loop rebuilds the atlas automatically when `FontAtlasRebuildPending()` is true.\n\n")

        f.write("Font variants for **bold** and **italic** are separate resources with the same face name but a different style bit. ")
        f.write("When bold/italic text is rendered and no styled variant is loaded, a fallback text colour is applied instead. ")
        f.write("Use `Font.SetDefaultStyleColor` to change that colour.\n\n")

        f.write("**Style constants:**\n\n")
        f.write("| Constant | Value | Meaning |\n|---|---|---|\n")
        f.write("| `FONT_STYLE_REGULAR` | `0` | Normal weight, no slant |\n")
        f.write("| `FONT_STYLE_BOLD` | `1` | Bold variant |\n")
        f.write("| `FONT_STYLE_ITALIC` | `2` | Italic variant |\n")
        f.write("\n")

        f.write("### Loading & Lifecycle\n\n")
        f.write("Fonts are loaded through the session-wide `Resource.SetLoader` callback with `type = RESOURCE_FONT`. ")
        f.write("The source string passed to the loader is the internal key `\"FaceName:size:style\"`. ")
        f.write("The loader must return a stream or raw string containing the TTF file bytes.\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `Font.Resolve(face, size [, bold [, italic]])` | `string, number, boolean?, boolean?` | `integer\\|nil` | Load and register a TTF font variant. If already cached returns the existing `luaId`. Returns `nil` if the loader returns nothing. The atlas rebuild happens on the *next* frame |\n")
        f.write("| `Font.GetId(face, size [, bold [, italic]])` | `string, number, boolean?, boolean?` | `integer\\|nil` | Cache lookup without triggering a load. Returns `nil` if not yet loaded |\n")
        f.write("| `Font.GetData(luaId)` | `integer` | `table\\|nil` | Returns `{ face, size, style, isBuilt }`. `isBuilt` is `false` for the one transitional frame before the first atlas build |\n")
        f.write("\n")

        f.write("### Default Font\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `Font.SetDefault([luaId])` | `integer?` | — | Set the ImGui default font to a loaded `FontResource`. Pass `0` or `nil` to restore the built-in default. Takes effect after the next atlas rebuild |\n")
        f.write("| `Font.SetDefaultStyleColor(color, style)` | `integer, integer` | — | Set the fallback text colour used when a bold/italic font variant is not loaded. `color` is `ImU32` (`0xAABBGGRR`). `style` is a `FONT_STYLE_*` bitmask |\n")
        f.write("\n")

        f.write("### Glyph Queries\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `Font.HasGlyph(codepoint)` | `integer` | `boolean` | Whether the current default font contains a glyph for the given Unicode codepoint. Uses `FindGlyphNoFallback` so a missing glyph returns `false` even if ImGui has a fallback character |\n")
        f.write("| `Font.HasGlyphIn(luaId, codepoint)` | `integer, integer` | `boolean` | Same as `Font.HasGlyph` but checks a specific `FontResource` by `luaId` rather than the active default font |\n")
        f.write("| `Font.GetGlyphCodepoints([luaId])` | `integer?` | `table\\|nil` | Returns a 1-based sequential array of all Unicode codepoints that have a real glyph in the font. Skips codepoint 0 and invisible zero-area glyphs. Omit `luaId` or pass `nil` to query the current default font. Returns `nil` if the font is not yet built |\n")
        f.write("\n")

        f.write("```lua\n")
        f.write("-- Register the loader so Font.Resolve can fetch TTF bytes:\n")
        f.write("Resource.SetLoader(function(type, source)\n")
        f.write("    -- source for fonts is 'FaceName:size:style', e.g. 'Roboto:18.0:0'\n")
        f.write("    if type == 4 then -- RESOURCE_FONT\n")
        f.write("        local face = source:match('^([^:]+)')\n")
        f.write("        return Stream.Open('fonts/' .. face .. '.ttf', 'rb')\n")
        f.write("    end\n")
        f.write("end)\n\n")
        f.write("-- Load regular and bold variants (both must have the same face name):\n")
        f.write("local fntId     = Font.Resolve('Roboto', 18)            -- regular\n")
        f.write("local fntBoldId = Font.Resolve('Roboto', 18, true)      -- bold\n")
        f.write("local fntItId   = Font.Resolve('Roboto', 18, false, true) -- italic\n\n")
        f.write("-- Make it the application default (takes effect next frame):\n")
        f.write("Font.SetDefault(fntId)\n\n")
        f.write("-- Change the fallback colour shown when bold variant is missing:\n")
        f.write("Font.SetDefaultStyleColor(0xFFFFD966, 1) -- golden yellow, FONT_STYLE_BOLD\n\n")
        f.write("-- Query whether a character is covered by the current font:\n")
        f.write("if not Font.HasGlyph(0x2713) then   -- U+2713 CHECK MARK\n")
        f.write("    print('check mark glyph missing from default font')\n")
        f.write("end\n")
        f.write("```\n\n")

        f.write("## Markdown API\n\n")
        f.write("Parses Markdown text into a `RESOURCE_MARKDOWN` entry in the ResourceCache. ")
        f.write("The result is an integer `luaId` — the same pattern as textures and audio. ")
        f.write("Parse once, render as many times per frame as you like. No re-parse overhead.\n\n")

        f.write("### Loading\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `Markdown.Parse(text)` | `string` | `integer\\|nil` | Parse Markdown from a raw string. Returns a `luaId` or `nil` on OOM |\n")
        f.write("| `Markdown.Parse(stream)` | stream | `integer\\|nil` | Parse Markdown from a stream userdata (e.g. `Stream.Open`). Returns a `luaId` or `nil` on failure |\n")
        f.write("| `Markdown.Destroy(luaId)` | `integer` | — | Remove from the ResourceCache and free all memory |\n")
        f.write("\n")

        f.write("### `renderer:MarkdownRender(luaId [, w [, h]])`\n\n")
        f.write("Renders the parsed Markdown document inside an `ImGui::BeginChild` scrolling region. ")
        f.write("Safe to call with a `nil` or invalid `luaId` — silently renders nothing.\n\n")
        f.write("- `luaId` — integer returned by `Markdown.Parse`.\n")
        f.write("- `w`, `h` *(optional, number)* — child region size in pixels. `0` = fill available space. Default `0`.\n\n")
        f.write("**Supported syntax:** `# H1` `## H2` `### H3`, `---` horizontal rule, `**bold**`, `*italic*`, `` `inline code` ``, ")
        f.write("fenced code blocks (` ``` `), bullet lists (`-` / `*`), ordered lists, nested lists, ")
        f.write("blockquotes (`>`), tables (`| col |`), images (`![alt](resourceId)`), links (`[text](url)`), plain paragraphs.\n\n")
        f.write("```lua\n")
        f.write("-- Parse once at startup or on first use:\n")
        f.write("local readmeId = Markdown.Parse(Stream.Open('README.md', 'rb'))\n\n")
        f.write("-- Parse inline strings (e.g. chat messages):\n")
        f.write("local messages = {}\n")
        f.write("for _, text in ipairs(chatlog) do\n")
        f.write("    messages[#messages + 1] = Markdown.Parse(text)\n")
        f.write("end\n\n")
        f.write("-- Inside renderFn — render multiple independently per frame:\n")
        f.write("renderer:MarkdownRender(readmeId)           -- full width\n")
        f.write("for i, mdId in ipairs(messages) do\n")
        f.write("    renderer:PushID(i)\n")
        f.write("    renderer:MarkdownRender(mdId)           -- each in its own scroll region\n")
        f.write("    renderer:PopID()\n")
        f.write("end\n\n")
        f.write("-- Cleanup when done:\n")
        f.write("Markdown.Destroy(readmeId)\n")
        f.write("for _, mdId in ipairs(messages) do Markdown.Destroy(mdId) end\n")
        f.write("```\n\n")

        f.write("## HTML API\n\n")
        f.write("Full HTML/CSS rendering via [litehtml](https://github.com/litehtml/litehtml). ")
        f.write("Requires `KITSUNE_IMGUI`. Parsed documents live in the ResourceCache as `RESOURCE_HTML` entries ")
        f.write("and are identified by an integer `luaId` — same pattern as textures, audio, and Markdown.\n\n")

        f.write("### Loading\n\n")
        f.write("HTML can be loaded from a raw string or from a `RESOURCE_GENERIC` blob (raw bytes stored by `Resource.Load`). ")
        f.write("The generic resource path is preferred for large files because it lets you reload/replace content without re-parsing from disk.\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `Html.Parse(html)` | `string` | `integer\\|nil` | Parse HTML from a raw string literal. Returns `luaId` or `nil` on failure |\n")
        f.write("| `Html.Parse(genericId)` | `integer` | `integer\\|nil` | Parse HTML from a `RESOURCE_GENERIC` resource id (from `Resource.Load`). Returns `luaId` or `nil` on failure |\n")
        f.write("| `Html.Destroy(luaId)` | `integer` | — | Remove from the ResourceCache, free the litehtml document and all associated memory |\n")
        f.write("\n")

        f.write("### Document Control\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `Html.Invalidate(luaId)` | `integer` | — | Force a full reflow on the next `renderer:Html()` call (e.g. after font atlas rebuild) |\n")
        f.write("| `Html.Reload(luaId, genericId)` | `integer, integer` | — | Re-parse the document from a (possibly updated) `RESOURCE_GENERIC` blob and force reflow |\n")
        f.write("\n")

        f.write("### Querying the DOM\n\n")
        f.write("Element tables returned by query functions have the shape `{ tag, id, class, href, handle, attrs }` ")
        f.write("where `handle` is an opaque integer used to reference the element in subsequent calls.\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `Html.Query(luaId, selector)` | `integer, string` | `table` | CSS selector — returns a sequential table of element tables |\n")
        f.write("| `Html.QueryOne(luaId, selector)` | `integer, string` | `table\\|nil` | Returns the first matching element table, or `nil` |\n")
        f.write("| `Html.QueryByHandle(luaId, handle)` | `integer, integer` | `table\\|nil` | Look up an element by opaque handle integer (as passed to event handlers) |\n")
        f.write("| `Html.SetAttr(luaId, handle, attr, value)` | `integer, integer, string, string` | — | Set an attribute on an element and force reflow |\n")
        f.write("\n")

        f.write("### Events\n\n")
        f.write("Events are fired synchronously during `renderer:Html()`. ")
        f.write("The handler receives the event type, the document `luaId`, and an opaque element `handle` integer.\n\n")
        f.write("| Function | Parameters | Notes |\n|---|---|---|\n")
        f.write("| `Html.SetEventHandler(luaId, fn)` | `integer, function` | Set callback `fn(eventtype, luaId, handle)`. `eventtype` is `\"click\"`, `\"mouseover\"`, or `\"mouseout\"` |\n")
        f.write("\n")

        f.write("### `renderer:Html(luaId [, w [, h]])`\n\n")
        f.write("Renders an HTML document inside the current ImGui window using the litehtml layout engine.\n\n")
        f.write("- `luaId` — integer returned by `Html.Parse`.\n")
        f.write("- `w` *(optional, number)* — render width in pixels. Defaults to `GetContentRegionAvail().x`.\n")
        f.write("- `h` *(optional, number)* — render height in pixels. `0` = auto (full document height). Default `0`.\n\n")
        f.write("Returns `true` on success, or `false, errmsg` on failure.\n\n")
        f.write("Reflowing is skipped when width is unchanged. Font handles are baked into the litehtml element tree at parse time — ")
        f.write("call `Html.Invalidate` after a font atlas rebuild to force a full re-parse.\n\n")
        f.write("Click events on non-`<a>` elements fire on the hovered element. ")
        f.write("`<a>` anchor clicks are routed through litehtml's `on_anchor_click` and fire on the anchor element.\n\n")
        f.write("```lua\n")
        f.write("-- Load from a file via RESOURCE_GENERIC:\n")
        f.write("local genericId = Resource.Load(Stream.Open('page.html', 'rb'), 'page.html')\n")
        f.write("local htmlId    = Html.Parse(genericId)\n\n")
        f.write("-- Or parse an inline string directly:\n")
        f.write("local inlineId = Html.Parse('<h1>Hello</h1><p>World</p>')\n\n")
        f.write("-- Attach event handler:\n")
        f.write("Html.SetEventHandler(htmlId, function(event, id, handle)\n")
        f.write("    if event == 'click' then\n")
        f.write("        local el = Html.QueryByHandle(id, handle)\n")
        f.write("        print('clicked', el and el.tag, el and el.id)\n")
        f.write("    end\n")
        f.write("end)\n\n")
        f.write("-- Inside renderFn:\n")
        f.write("renderer:Html(htmlId)        -- auto height, full width\n")
        f.write("renderer:Html(inlineId, 400) -- 400px wide, auto height\n\n")
        f.write("-- Multiple independent documents side-by-side:\n")
        f.write("local cw, ch = renderer:GetContentRegionAvail()\n")
        f.write("renderer:BeginChild('##left', cw/2, ch, true)\n")
        f.write("    renderer:Html(htmlId)\n")
        f.write("renderer:EndChild()\n")
        f.write("renderer:SameLine()\n")
        f.write("renderer:BeginChild('##right', cw/2, ch, true)\n")
        f.write("    renderer:Html(inlineId)\n")
        f.write("renderer:EndChild()\n\n")
        f.write("-- Cleanup:\n")
        f.write("Html.Destroy(htmlId)\n")
        f.write("Html.Destroy(inlineId)\n")
        f.write("```\n\n")
