...
def main():
    ...
    with open(OUT_RENDERER, "w", encoding="utf-8") as f:
        ...
        f.write("\t\tNamedKitsuneFunction* node = (NamedKitsuneFunction*)malloc(sizeof(NamedKitsuneFunction));\n")
    ...
    with open(OUT_DOCS, "w", encoding="utf-8") as f:
        ...
        f.write("### `Imgui.Console(visible)`\n\n")
        f.write("Shows or hides the console window. **Windows only** — no-op on Linux.\n\n")

        f.write("### Window functions\n\n")
        f.write("All return `nil` if called before `Imgui.Start`.\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
        f.write("| `Imgui.GetWindowWidth()` | — | `integer` | Current window width in pixels |\n")
        f.write("| `Imgui.GetWindowHeight()` | — | `integer` | Current window height in pixels |\n")
        f.write("| `Imgui.GetWindowX()` | — | `integer` | Window X position on screen |\n")
        f.write("| `Imgui.GetWindowY()` | — | `integer` | Window Y position on screen |\n")
        f.write("| `Imgui.SetWindowSize(w, h)` | `integer, integer` | — | Resize the window |\n")
        f.write("| `Imgui.SetWindowPosition(x, y)` | `integer, integer` | — | Move the window |\n")
        f.write("| `Imgui.SetWindowTitle(title)` | `string` | — | Change the window title bar text |\n")
        f.write("| `Imgui.IsMinimized()` | — | `boolean` | Whether the window is minimised |\n")
        f.write("| `Imgui.IsFocused()` | — | `boolean` | Whether the window has input focus |\n")
        f.write("| `Imgui.SetFullscreen(enabled)` | `boolean` | — | Toggle borderless fullscreen |\n")
        f.write("| `Imgui.GetMonitor()` | — | `index, name, x, y, width, height, refreshRate` | Info about the monitor the window is currently on |\n")
        f.write("\n")
        f.write("## Supported Methods\n\n")
        f.write("| Signature | Returns |\n|---|---|\n")
        for (sig, returns) in fn_docs:
            f.write(f"| `{sig}` | `{returns}` |\n")
        f.write("\n## Hand-written Overrides\n\n")
        f.write("These functions are implemented manually in `ImguiRenderer.cpp`:\n\n")
