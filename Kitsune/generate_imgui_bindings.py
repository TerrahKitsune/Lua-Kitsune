...
def main():
    ...
    with open(OUT_RENDERER, "w", encoding="utf-8") as f:
        ...
        f.write("\t\tKitsuneNamedFunction* node = (KitsuneNamedFunction*)malloc(sizeof(KitsuneNamedFunction));\n")
    ...
    with open(OUT_DOCS, "w", encoding="utf-8") as f:
        ...
        f.write("## SDL API\n\n")
        f.write("Window and display functions. All return `nil` if called before `Imgui.Start`.\n\n")
        ...
        f.write("| `SDL.GetMonitor()` | — | `index, name, x, y, width, height, refreshRate` | Info about the monitor the window is currently on |\n")
        f.write("\n")

        f.write("## Win32 API\n\n")
        f.write("Console window functions. All are no-op on Linux.\n\n")
        f.write("| Function | Parameters | Returns | Notes |\n|---|---|---|---|\n")
