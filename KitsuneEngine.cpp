	...
	KITSUNE_API size_t KitsuneCleanup() {
		...
		if (state) {
			...
			if (state->L) {
				...
				// Kill the app-level AliveToken before GC/close so any coroutine still
				// holding a reference to it sees it as dead on the next tick.
				lua_alivetoken_app_kill(state->L);
