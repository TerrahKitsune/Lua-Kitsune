...
void* lua_json_null(void) { return &g_json_null; }
...
if (j->out) { gff_free(j->out); j->out = NULL; }
if (j->rec) { gff_free(j->rec); j->rec = NULL; }
return 0;
}
...
if (j->outLen + need <= j->outCap) return;
size_t cap = j->outCap ? j->outCap * 2 : 512;
while (cap < j->outLen + need) cap *= 2;
char* p = (char*)gff_realloc(j->out, cap);
if (!p) luaL_error(L, "Json: out of memory");
j->out    = p;
j->outCap = cap;
}
...
if (j->rec[i] == addr) luaL_error(L, "Json: recursion detected");
...
static void rec_pop(LuaJson* j) { if (j->recLen > 0) j->recLen--; }
...
if (!j->pretty) return;
jbuf_emitc(j, L, '\n');
for (int i = 0; i < depth; i++) jbuf_emit(j, L, "  ", 2);
}
...
if (isnan(d)) { jbuf_emitlit(j, L, "null");    return; }
if (isinf(d)) {
if (d > 0) jbuf_emitlit(j, L, "1e+9999");
else       jbuf_emitlit(j, L, "-1e+9999");
return;
}
char buf[64];
...
// An empty table is NOT a sequence (encodes as {}).
...
if (k < 1 || k > n)        { lua_pop(L, 1); return false; }
count++;
}
return count == n;
}
...
if (i > 1) jbuf_emitc(j, L, ',');
enc_indent(j, L, depth + 1);
lua_rawgeti(L, tbl, i);
enc_value(j, L, depth + 1);
lua_pop(L, 1);
}
if (n > 0) enc_indent(j, L, depth);
jbuf_emitc(j, L, ']');
} else {
jbuf_emitc(j, L, '{');
bool first = true;
...
jbuf_emitlit(j, L, ":");
if (j->pretty) jbuf_emitc(j, L, ' ');
enc_value(j, L, depth + 1);  // encode value at -1
lua_pop(L, 1);               // pop value
}
if (!first) enc_indent(j, L, depth);
jbuf_emitc(j, L, '}');
}
...
break;
case LUA_TBOOLEAN:
if (lua_toboolean(L, -1)) jbuf_emitlit(j, L, "true");
else                      jbuf_emitlit(j, L, "false");
break;
case LUA_TNUMBER:
enc_number(j, L);
break;
...
static char jread_next(LuaJson* j, lua_State* L) {
if (j->ungetLen > 0) return j->unget[--j->ungetLen];
if (j->srcPos >= j->srcLen) return '\0';
char c = j->src[j->srcPos++];
if (c == '\n') { j->errLine++; j->errCol = 0; }
else           { j->errCol++; }
return c;
}

static void jread_unget(LuaJson* j, char c) {
if (j->ungetLen < 8) j->unget[j->ungetLen++] = c;
}

static char jread_skip(LuaJson* j, lua_State* L) {
char c;
while ((c = jread_next(j, L)) != '\0')
if (c != ' ' && c != '\t' && c != '\r' && c != '\n') return c;
return '\0';
}
...
luaL_buffinit(L, &b);
for (;;) {
char c = jread_next(j, L);
if (c == '\0') luaL_error(L, "Json: unterminated string at line %zu", j->errLine + 1);
if (c == '"')  break;
if (c != '\\') { luaL_addchar(&b, c); continue; }

char esc = jread_next(j, L);
switch (esc) {
...
char hex[5] = { 0 };
for (int k = 0; k < 4; k++) {
hex[k] = jread_next(j, L);
if (!hex[k]) luaL_error(L, "Json: unexpected end in \\u escape");
}
unsigned int cp = (unsigned int)strtoul(hex, NULL, 16);
// Handle UTF-16 surrogate pair
if (cp >= 0xD800 && cp <= 0xDBFF) {
char p1 = jread_next(j, L);
char p2 = jread_next(j, L);
if (p1 == '\\' && p2 == 'u') {
char hex2[5] = { 0 };
for (int k = 0; k < 4; k++) hex2[k] = jread_next(j, L);
...
if (end != buf && *end == '\0') { lua_pushinteger(L, (lua_Integer)v); return; }
}
...
lua_newtable(L);
char c = jread_skip(j, L);
if (c == '}') return;
jread_unget(j, c);
for (;;) {
c = jread_skip(j, L);
if (c != '"') luaL_error(L, "Json: expected string key at line %zu", j->errLine + 1);
dec_string(j, L);          // push key
c = jread_skip(j, L);
if (c != ':') luaL_error(L, "Json: expected ':' at line %zu", j->errLine + 1);
dec_value(j, L);           // push value
lua_rawset(L, -3);         // table[key] = value
c = jread_skip(j, L);
if (c == '}') break;
if (c != ',') luaL_error(L, "Json: expected ',' or '}' at line %zu", j->errLine + 1);
}
}
...
case 't': {
char r = jread_next(j, L), u = jread_next(j, L), e = jread_next(j, L);
if (r != 'r' || u != 'u' || e != 'e')
...
case 'f': {
char a = jread_next(j, L), l = jread_next(j, L),
     s = jread_next(j, L), e = jread_next(j, L);
if (a != 'a' || l != 'l' || s != 's' || e != 'e')
luaL_error(L, "Json: invalid token at line %zu", j->errLine + 1);
lua_pushboolean(L, 0);
break;
...
case 'n': {
char u = jread_next(j, L), l1 = jread_next(j, L), l2 = jread_next(j, L);
if (u != 'u' || l1 != 'l' || l2 != 'l')
luaL_error(L, "Json: invalid token at line %zu", j->errLine + 1);
lua_pushlightuserdata(L, lua_json_null());  // push the null sentinel
break;
}
case '\0':
...
break;
default:
if (c == '-' || (c >= '0' && c <= '9')) { dec_number(j, L, c); break; }
...
luaL_checkany(L, 2);
enc_reset(j);
lua_pushvalue(L, 2);        // copy of the value to encode at top
enc_value(j, L, 0);
lua_pop(L, 1);
...
// =============================================================================
// Static one-shot wrappers
// =============================================================================

// Json.Decode(str)
int lua_json_static_decode(lua_State* L) {
lua_json_push(L);  // [str(1), j(2)]
lua_insert(L, 1);  // [j(1),  str(2)]
return lua_json_decode(L);
}

// Json.Encode(value [, pretty])
int lua_json_static_encode(lua_State* L) {
int      pretty = lua_toboolean(L, 2);
lua_settop(L, 1);                    // [value(1)]
LuaJson* j      = lua_json_push(L);  // [value(1), j(2)]
j->pretty       = pretty;
