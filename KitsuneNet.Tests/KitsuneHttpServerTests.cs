...
public sealed class KitsuneHttpServerTests
{
    ...
    public async Task Server_SSE_LuaClient_CanConsumeStreamingEvents()
    {
        ...
        if (await new KitsuneEngine().ExecuteStringAsync("return type(HttpClient)") == "nil")
        {
    ...
    private static async Task<(string Data, WebSocketMessageType Type)> WsCsEchoOnce(string url, string message, WebSocketMessageType sendType = WebSocketMessageType.Text)
    {
        using var ws = new ClientWebSocket();
